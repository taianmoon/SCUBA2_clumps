...
scidata_flux = hdulist_flux[0].data  #================flux: unit of mJy for Todd's clusters
...
#for i in range(100):   #==============================================================================choose intial random position number: HERE
    x_tmp=random.randint(1,NAXIS1_flux-2)
    y_tmp=random.randint(1,NAXIS2_flux-2)
    if sensitivity_switch==1:
        if math.sqrt(pow(x_tmp-(NAXIS1_flux/2.0), 2.0)+pow(y_tmp-(NAXIS2_flux/2.0), 2.0)) <= radius_sources/(60.0*abs(CDELT1_flux)) and np.isnan(scidata_flux[0, y_tmp, x_tmp])==False:
...
for i in range (0, len(random_positions_x)):  #=================loop over 10 random positions, specify their flux values
    #if np.isnan(scidata_flux[0, random_positions_y[i], random_positions_x[i]])==False:
        scidata_flux[0, random_positions_y[i], random_positions_x[i]]=random_flux[rf]
...
#hdul.writeto('../'+cluster_list[l]+'/mock_'+str(random_flux_mJy_int[rf])+'mJy_'+cluster_list[l]+'_flux_170919.fits')
hdul.writeto('./190911_wholemap/mock_'+str(random_flux_mJy_int[rf])+'mJy_'+cluster_list[l]+'_flux_170919.fits')
...
#hdul_sn.writeto('../'+cluster_list[l]+'/mock_'+str(random_flux_mJy_int[rf])+'mJy_'+cluster_list[l]+'_snr_170919.fits')
hdul_sn.writeto('./190911_wholemap/mock_'+str(random_flux_mJy_int[rf])+'mJy_'+cluster_list[l]+'_snr_170919.fits')
