...
for l in range(0, len(cluster_list)):
    ...
    template_file = open("./edgesourcedelete_deboost_190812/"+cluster_list[l]+"_3p5sigma_deboost.cat", "r")    #=========================================HERE
    ...
    for i in range(0, len(name)):
        ...
        flux_error_mJy_deboost_confusion.append(math.sqrt(pow(0.7, 2)+pow(flux_error_mJy_deboost[i], 2)))
        SN_ratio_confusion.append(flux_mJy_perbeam_deboost[i]/(math.sqrt(pow(0.7, 2)+pow(flux_error_mJy_deboost[i], 2))))
        flux_error_mJy_deboost_confusion_calibration.append(math.sqrt(pow(math.sqrt(pow(0.7, 2)+pow(flux_error_mJy_deboost[i], 2)), 2)+pow(flux_mJy_perbeam_deboost[i]*0.05, 2)))
        flux_error_mJy_deboost_calibration.append(math.sqrt(pow(flux_error_mJy_deboost[i], 2)+pow(flux_mJy_perbeam_deboost[i]*0.05, 2)))
    ...
    mock_catalogue=np.column_stack((name, RA, Dec, SN_ratio, flux_mJy_perbeam_deboost, flux_error_mJy_deboost_calibration))

    np.savetxt('./catalogue_for_thesis_191003/'+cluster_list[l]+'.cat', mock_catalogue, delimiter=' ', header="ID RA Dec SNR flux_density_mJy flux_density_err_mJy")



    #mock_catalogue_ds9=np.column_stack((RA, Dec))

    #np.savetxt('./edgesourcedelete_deboost_190812/'+cluster_list[l]+'_3p5sigma_deboost.reg', mock_catalogue_ds9, delimiter=' ') #=================================================HERE
