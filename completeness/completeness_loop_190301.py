...
cluster_list=['NGP9']  #can't do more clusters because of code constraints #=========================================================================================HERE
sensitivity_switch=1 #===========================HERE: 1--confine to central region, 2--divide the sensitivity area curve
realization_times=1500  #=========================================================================================HERE (1500)
radius_sources=4.0  #=========================================================================================HERE:arcmin, radius (exclude edge 30" for Gaussian value specifications and visual removal in real catalogues) ((350.0-30.0)/60.0)
...
def two_d_gaussian(peak_flux, peak_x, peak_y, given_x, given_y, pixel_size_x, pixel_size_y): #coordinates are in pixel coordinates. Pixel sizes in degrees
    ...
    return peak_flux*math.exp(-(pow(((peak_x - given_x)*pixel_size_x),2)/(2.0*pow(14.6/(2.3*3600),2)) + pow((peak_y - given_y)*pixel_size_y,2)/(2.0*pow(14.6/(2.3*3600),2)) )) #14.6" is the SCUBA-2 beam FWHM =3.2 sigma
