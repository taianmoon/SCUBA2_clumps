...
for i in range (0, len(random_positions_x)):  #=================loop over 10 random positions, specify their flux values
    if np.isnan(scidata_flux[0, random_positions_y[i], random_positions_x[i]])==False:
        scidata_flux[0, random_positions_y[i], random_positions_x[i]]=random_flux[rf]  #peak (central) pixel of 2D-Gaussian
        
        for pvx in range(0,5):
            for pvy in range(0,5):
        
                scidata_flux[0, random_positions_y[i]+pvx, random_positions_x[i]+pvy]=two_d_gaussian(random_flux[rf], random_positions_y[i], random_positions_x[i], random_positions_y[i]+pvx, random_positions_x[i]+pvy, abs(CDELT2_flux), abs(CDELT1_flux))
                scidata_flux[0, random_positions_y[i]-pvx, random_positions_x[i]-pvy]=two_d_gaussian(random_flux[rf], random_positions_y[i], random_positions_x[i], random_positions_y[i]-pvx, random_positions_x[i]-pvy, abs(CDELT2_flux), abs(CDELT1_flux))
                scidata_flux[0, random_positions_y[i]+pvx, random_positions_x[i]-pvy]=two_d_gaussian(random_flux[rf], random_positions_y[i], random_positions_x[i], random_positions_y[i]+pvx, random_positions_x[i]-pvy, abs(CDELT2_flux), abs(CDELT1_flux))
                scidata_flux[0, random_positions_y[i]-pvx, random_positions_x[i]+pvy]=two_d_gaussian(random_flux[rf], random_positions_y[i], random_positions_x[i], random_positions_y[i]-pvx, random_positions_x[i]+pvy, abs(CDELT2_flux), abs(CDELT1_flux))
