...
if cluster_list[l]=='S2CLS':
    ...
    flux_mjy_err_pre=response_curve[:,14]    #=========================================================HERE: 8 for Todd's cluster, 7 for our clusters
...
for i in range(0, len(flux_mjy_pre)):
    if cluster_list[l]=='NGP6':  #getting rid of faintest sources in NGP6 since area correction is not reliable
        if sn_ratio_confusion_pre[i] >= sn_want and flux_mjy_pre[i]>5.0:
...
if cluster_list[l]=='S2CLS':
    ...
    hdulist_noise = fits.open('../'+cluster_list[l]+'/'+cluster_list[l]+'_flux_170919.fits')      #================================================================================HERE
...
for k in range(0, len(flux_mjy_sorted_pre)):
    ...
    area_source.append(np.interp(flux_mjy_sorted_pre[k]*(1.0+0.2*pow(sn_ratio_sorted[k]/5.0,-2.3)), noise_value_array_sorted, noise_area)) #Have to use flux "without" de-boosted, so boost back here!
...
ax.errorbar(bin_flux, n_cum, yerr=completeness_err, color='blue',fmt='o', capsize=5, elinewidth=2,markeredgewidth=2) #removed on 180703
ax.plot(bin_flux, n_cum, label=cluster_list[l])
ax.scatter(bin_flux, n_cum, s=40.0)
...
if binsize==1:
    np.savetxt('./cum_counts_'+cluster_list[l]+'_bin1.cat', mock_catalogue, delimiter=' ', header='bin_flux n_cum completeness_err')
if binsize==2:
    np.savetxt('./cum_counts_'+cluster_list[l]+'_bin2.cat', mock_catalogue, delimiter=' ', header='bin_flux n_cum completeness_err')
